...
#define VERIFY_WORLD(func)                                         \
    ...
    void BasicSceneBuilder::WorldBegin(FileLoc loc)
    {
        ...
        if (renderOrientationCnt > 1)
        {
            scene->SetOptions(filter, film, cameras, sampler, integrator, accelerator, &currentCamera);
        }
        else
        {
        ...
        Float ux, Float uy, Float uz, FileLoc loc)
    {
        ...
        camera.pos = Point3f(ex, ey, ez);
        camera.look = Point3f(lx, ly, lz);
        camera.up = Vector3f(ux, uy, uz);
    ...
    void BasicScene::SetOptions(SceneEntity filter, SceneEntity film,
        const std::vector<CameraSceneEntity>& cameras, SceneEntity sampler,
        SceneEntity integ, SceneEntity accel, int* currentCamera)
    {
        // Store information for specified integrator and accelerator
        filmColorSpace = film.parameters.ColorSpace();
        integrator = integ;
